...
const get = (m: Map<string, Map<string, number>>, a: string, b: string) => m.get(a)?.get(b) ?? 0;
...
function bestSamePairing(players: Player[], state: PairingState): Array<[string, string]> {
...
function applyRests(state: PairingState, all: Player[], played: Set<string>) {
  for (const p of all) {
    if (!played.has(p.id)) state.rests.set(p.id, (state.rests.get(p.id) ?? 0) + 1);
  }
}

export function generateSchedule(t: Tournament, tr: Translate = germanFallback): ScheduleResult {
  ...
  for (let r = 0; r < t.rounds; r++) {
    ...
    if (t.allowPartialFinalRound && effectiveCourts < courtsPossible && !partialWarned) {
    ...
    if (t.mode === "mixed") {
      ...
      rounds.push({
        index: r + 1,
        matches,
        resting: t.players.filter((p) => !playedSet.has(p.id)).map((p) => p.id),
      });
      matchesGenerated += matches.length;
    } else {
      const pool = t.mode === "women" ? women : t.mode === "men" ? men : t.players;
      ...
      for (const m of matches) {
        for (const id of m.teamA.players) playedSet.add(id);
        for (const id of m.teamB.players) playedSet.add(id);
      }
      applyMatches(state, matches);
      applyRests(state, t.players, playedSet);
      rounds.push({

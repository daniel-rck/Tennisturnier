...
function ensure(map: Map<string, MutableAgg>, id: string, name: string): MutableAgg {
  let agg = map.get(id);
  if (!agg) {
    agg = {
      playerId: id,
...
export function computePlayerStats(t: Tournament): PlayerStat[] {
  ...
  for (const round of t.schedule) {
    for (const m of round.matches) {
      ...
      recordMatch(aggs, m.teamA.players, m.teamB.players, m.scoreA, m.scoreB, nameOfPlayer);
    }
  }
  ...
  for (const e of t.entries) {
    for (const member of e.members) {
      ...
      if (!playerKeys.has(key)) playerKeys.set(key, { id: key, name: trimmed });
    }
  }
  const nameOfEntryMember = (id: string) => playerKeys.get(id)?.name ?? id.replace(/^entry:/, "");
  ...
  for (const agg of aggs.values()) {
    ...
    const partnerNameOf = (id: string) => playerNames.get(id) ?? playerKeys.get(id)?.name ?? id;
    results.push({

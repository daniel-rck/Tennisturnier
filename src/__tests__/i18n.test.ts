...
describe('i18n', () => {
  it('English table covers every German key', () => {
    const missing = (Object.keys(de) as Array<keyof typeof de>).filter(
      (k) => !(k in en),
    )
  ...
  it('does not have extra keys in English', () => {
    const extra = (Object.keys(en) as Array<keyof typeof de>).filter(
      (k) => !(k in de),
    )
  ...
  it('every English value is non-empty', () => {
    const empty = (Object.keys(en) as Array<keyof typeof en>).filter(
      (k) => en[k] === '',
    )

...
export function PrintView({ tournament }: Props) {
  ...
  const hasContent =
    ...
    ((f === "groups" || f === "groups-ko") && tournament.entries.length > 0) ||
        ...
        <div className="bg-surface p-6 rounded-md border border-border">
          <header className="mb-4 border-b border-border-strong pb-3">
            ...
            <p className="text-sm text-fg-muted">
              ...
              {f === "rotation" && ` · ${t("print.subtitleRounds", { count: tournament.rounds })}`}
              {" · "}
              {f === "rotation"
    ...
    <>
      {t.schedule.map((round) => (
        <section key={round.index} className="mb-5">
          <h2 className="text-lg font-semibold mb-2">{tr("schedule.round", { n: round.index })}</h2>
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-surface-sunken text-left">
                <th className="border border-border-strong px-2 py-1 w-16">
                  {tr("print.col.court")}
                </th>
            ...
            <tbody>
              {round.matches.map((m) => (
                <tr key={m.court}>
                  <td className="border border-border-strong px-2 py-1 font-medium">{m.court}</td>
  ...
  return (
    <>
      ...
      {groups.map((group, gi) => {
        ...
        return (
          <section key={groupNum} className="mb-5">
            ...
            <table className="w-full text-xs border-collapse mb-2">
              <thead>
                <tr className="bg-surface-sunken text-left">
                  ...
                  <th className="border border-border-strong px-1 py-1">
                    {tr("ranking.col.name")}
                  </th>
              ...
              <tbody>
                {standings.map((s) => (
                  <tr key={s.entryId}>
                    ...
                    <td className="border border-border-strong px-1 py-1">{s.name}</td>
                    <td className="border border-border-strong px-1 py-1 text-right">{s.played}</td>
                    <td className="border border-border-strong px-1 py-1 text-right">{s.wins}</td>
...
function BracketPrint({ t }: { t: Tournament }) {
  const { t: tr } = useTranslation();
  const entryName = (id: string) => t.entries.find((e) => e.id === id)?.name ?? "?";
          ...
          {roundNumbers.map((rn) =>
            rounds.get(rn)?.map((m) => (
              <tr key={m.matchId}>
                ...
                <td className="border border-border-strong px-1 py-1">{m.pendingA}</td>
                <td className="border border-border-strong px-1 py-1 text-center tabular-nums">
                  {m.isByeMatch
                    ? tr("bracket.bye")
                    : m.scoreA != null && m.scoreB != null
                ...
                <td className="border border-border-strong px-1 py-1">{m.pendingB}</td>
              </tr>
            )),
          )}
        </tbody>
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id}>
              ...
              <td className="border border-border-strong px-2 py-1 text-right">{r.played}</td>
              <td className="border border-border-strong px-2 py-1 text-right">{r.wins}</td>
              <td className="border border-border-strong px-2 py-1 text-right">{r.draws}</td>
              <td className="border border-border-strong px-2 py-1 text-right">{r.losses}</td>
...
function GroupsRankingPrint({ t }: { t: Tournament }) {
  ...
  groups.forEach((group, gi) => {
    ...
    const hasDecidedMatch = matches.some((m) => m.scoreA != null && m.scoreB != null);
        ...
        <tbody>
          ...
          {podium.runnerUp && (
            <tr>
              ...
              <td className="border border-border-strong px-2 py-1">{podium.runnerUp}</td>
            </tr>
          )}
          {podium.thirds.length > 0 && (
            <tr>
              ...
              <td className="border border-border-strong px-2 py-1">{podium.thirds.join(", ")}</td>
            </tr>
          )}
        </tbody>
      </table>

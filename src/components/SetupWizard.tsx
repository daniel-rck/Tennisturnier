...
export function SetupWizard(props: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      {step === 1 && <StepDetails {...props} onBack={() => setStep(0)} onFinish={props.onFinish} />}
    </div>
  );
}
        ...
        return (
          <li key={label} className="flex-1 flex items-center gap-2 min-w-0">
            ...
            {i < labels.length - 1 && (
              <span
                aria-hidden
                className={`h-px flex-1 ${step > (i as Step) ? "bg-brand-soft" : "bg-border"}`}
              />
            )}
          </li>
        );
  ...
  return (
    <div className="space-y-5">
      ...
      <Card variant="flat" className="p-4 space-y-2">
        <label htmlFor="wizard-tournament-name" className="block text-sm font-medium text-fg">
          {t("setup.tournamentName")}
        </label>
        <input
          id="wizard-tournament-name"
          type="text"
          value={name}
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-fg-muted mt-0.5">{t(FORMAT_DESC_KEYS[f])}</div>
                  </div>
                </div>
              </button>
            );
...
function StepDetails(props: Props & { onBack: () => void; onFinish: () => void }) {
  const { t } = useTranslation();
  const {
    format,
    entryFormat,
...
function NumberField({
  ...
  return (
    <div>
      <label htmlFor={fieldId} className="block text-sm font-medium text-fg mb-1">
        {label}
      </label>
      <NumberInput
        id={fieldId}

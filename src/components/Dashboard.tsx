  ...
  return (
    <div className="space-y-5">
      ...
      {(f === "groups" || f === "groups-ko") && (
        <GroupsDashboard tournament={tournament} isOwner={isOwner} onScore={onGroupScore} />
      )}
      {(f === "knockout" || f === "groups-ko") && (
        <BracketDashboard tournament={tournament} isOwner={isOwner} onScore={onBracketScore} />
      )}
    </div>
  );
}
      ...
      <div className="bg-court-pattern px-5 py-5 text-cream">
        ...
        <div className="mt-4 space-y-1.5">
          <div className="flex items-baseline justify-between text-xs text-cream/85">
            ...
            <span className="tabular font-medium">{t("dashboard.progress", { done, total })}</span>
          </div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="serif text-xl font-semibold">{t("schedule.round", { n: round.index })}</h2>
        <button
          type="button"
...
function bracketRoundLabel(
  ...
  t: (
    k: "bracket.roundFinal" | "bracket.roundSemi" | "bracket.roundQuarter" | "bracket.roundEighth",
  ) => string,
): string {

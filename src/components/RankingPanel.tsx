...
function RotationRankingView({ tournament }: Props) {
  ...
  const completed = tournament.schedule
    .flatMap((r) => r.matches)
    .filter((m) => m.scoreA != null && m.scoreB != null).length;
  const total = tournament.schedule.flatMap((r) => r.matches).length;

  const showPerGender = tournament.mode === "mixed" && tournament.perGenderRanking;
  ...
  return (
    <div className="space-y-6">
      ...
      <RankingSection title={showPerGender ? t("ranking.overall") : undefined} rows={rows} />
...
function RankingSection({ title, rows }: { title?: string; rows: RotationRow[] }) {
  const podium = rows.slice(0, 3);
  return (
    <section className="space-y-4">
      {title && (
        <h2 className="serif text-xl font-semibold border-b border-border pb-1.5">{title}</h2>
      )}
      {podium.length >= 3 && <Podium entries={podium.map(rowToPodiumEntry)} size="compact" />}
  ...
  return (
    <div className="space-y-4">
      ...
      {groups.map((group, gi) => {
        ...
        return (
          <Card key={groupNum} variant="base" className="p-4">
...
function GroupsKoRanking({ tournament }: Props) {
  ...
  const resolved = useMemo(
    () =>
      resolveBracket(tournament.bracket, entryName, (g, r) => groupWinnerMap.get(`${g}|${r}`), t),
    ...
    <div className="space-y-4">
      {champion ? (
        <Card variant="hero" className="bg-court-pattern text-cream text-center p-6">
          <div className="text-5xl mb-2 animate-gold-glow inline-block rounded-full px-4 py-2">
            🏆
          </div>
          <div className="text-xs uppercase tracking-wider font-semibold text-cream/80">
            {t("ranking.champion")}
          </div>
          <div className="serif text-3xl sm:text-4xl font-semibold mt-1">{champion}</div>
        </Card>
      ) : (
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <Card variant="base" className="p-3">
          ...
          <div className="serif font-semibold text-base">{runnerUp ?? t("common.dash")}</div>
        </Card>
        <Card variant="base" className="p-3">
          <div className="text-xs uppercase tracking-wider text-fg-muted mb-1">
        ...
        <tbody>
          {rows.map((r) => (
            ...
            >
              ...
              <td className="px-2 py-2 2xl:py-3 text-right tabular text-fg-muted">{r.draws}</td>
              <td className="px-2 py-2 2xl:py-3 text-right tabular text-danger-fg">{r.losses}</td>

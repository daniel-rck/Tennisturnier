...
function strikeAttack(c: AudioContext, start: number, duration: number, amp: number): void {
...
const VARIANTS: Record<BellVariant, VariantSpec> = {
  classic: {
    loopInterval: 1.8,
    schedule: (c, start) => playPartials(c, start, 880, CLASSIC_PARTIALS, 0.25),
  },
  boxing: {
    loopInterval: 1.6,
    schedule: (c, start) => {
...
export async function ringBell(variant: BellVariant = "classic", times = 3): Promise<void> {
  const c = await ensureRunning();
  if (!c) return;
